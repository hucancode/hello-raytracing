...
impl From<Mesh> for Tree {
  fn from(mesh: Mesh) -> Self {
    ...
    while let Some((i,j,depth)) = q.pop_front() {
      ...
      triangles[i..min(j, m)]
        .sort_by(|(center_a,_), (center_b,_)| {
          center_a[depth%3].partial_cmp(&center_b[depth%3]).unwrap_or(Ordering::Equal)
        });
      let m = (i+j)/2;
      q.push_back((i,m,depth+1));
      q.push_back((m,j,depth+1));
    }
    let vertices: Vec<[f32;4]> = mesh.vertices.iter()
      .map(|v| v.position)
      .collect();
    let normals: Vec<[f32;4]> = triangles.iter()
    ...
    for i in 0..triangles.len() {
      let mut j = (i+n)/2;
      while j > 0 {
        let (_, triangle) = triangles[i];
        nodes[j].refit_xyzw(vertices[triangle[0] as usize]);
        nodes[j].refit_xyzw(vertices[triangle[1] as usize]);
        nodes[j].refit_xyzw(vertices[triangle[2] as usize]);
        j /= 2;
      }
    }
    let indices = triangles.into_iter()
